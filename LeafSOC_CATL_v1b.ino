...
#define V_SOC_5   334.67F               // Уровень напряжения ВВБ при SOC 5%
#define V_SOC_10  338.06F               // Уровень напряжения ВВБ при SOC 10%
#define V_SOC_20  346.34F               // Уровень напряжения ВВБ при SOC 20%
//#define V_SOC_25  348.00F               // Уровень напряжения ВВБ при SOC 25%  \
//#define V_SOC_30  349.34F               // Уровень напряжения ВВБ при SOC 30%   |Закомментировано, потому что расчетные значения практически совпадают с данными уровнями напряжения и расчеты этих уровней избыточны
//#define V_SOC_35  350.64F               // Уровень напряжения ВВБ при SOC 35%   |
//#define V_SOC_40  352.19F               // Уровень напряжения ВВБ при SOC 40%  /
#define V_SOC_50  354.54F               // Уровень напряжения ВВБ при SOC 50%
#define V_SOC_60  364.56F               // Уровень напряжения ВВБ при SOC 60%
//#define V_SOC_75  371.83F               // Уровень напряжения ВВБ при SOC 75%
//#define V_SOC_80  384.00F               // Уровень напряжения ВВБ при SOC 80%
#define V_SOC_90  386.08F               // Уровень напряжения ВВБ при SOC 90%
...
float VsocPct20;                        // Доля в вольтах от общего напряжения в одном проценте SOC в интервале от 10% до 20%
float VsocPct50;                        // Доля в вольтах от общего напряжения в одном проценте SOC в интервале от 20% до 50%
float VsocPct60;                        // Доля в вольтах от общего напряжения в одном проценте SOC в интервале от 50% до 60%
//float VsocPct80;                        // Доля в вольтах от общего напряжения в одном проценте SOC в интервале от 60% до 80%
float VsocPct90;                        // Доля в вольтах от общего напряжения в одном проценте SOC в интервале от 60% до 90%
  ...
  {                                             // Читаем и собираем данные из EV-CAN шины
   ...
   if (Page >=1 && Page <= 5)
    ...
    { 
        ...
        VsocPct20   = (V_SOC_20 - V_SOC_10)/10.0F;
        VsocPct50   = (V_SOC_50 - V_SOC_20)/30.0F;
        VsocPct60   = (V_SOC_60 - V_SOC_50)/10.0F;
      //  VsocPct80   = (V_SOC_80 - V_SOC_60)/20.0F;
        VsocPct90   = (V_SOC_90 - V_SOC_60)/30.0F;
        VsocPct100  = (V_SOC_100 - V_SOC_90)/10.0F;
         ...
         else if (BattVolts > V_SOC_60) 
         {
           ActSocPct = ((BattVolts - V_SOC_60) / VsocPct90 + 60) / 1.0F;
         }
         else if (BattVolts > V_SOC_50) 
         {
           ActSocPct = ((BattVolts - V_SOC_50) / VsocPct60 + 50) / 1.0F;
         }
         else if (BattVolts > V_SOC_20) 
         {
           ActSocPct = ((BattVolts - V_SOC_20) / VsocPct50 + 20) / 1.0F;
         }
         else if (BattVolts > V_SOC_10) 
         {
           ActSocPct = ((BattVolts - V_SOC_10) / VsocPct20 + 10) / 1.0F;
         }
